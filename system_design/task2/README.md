# ТигрБанк - Модуль «Учет финансов»

## 1. Предметная область

Программа моделирует модуль учета личных финансов для банковского приложения ТигрБанк.
...
- Фиксировать **финансовые операции** (доход/расход) с привязкой к счету, категории, дате и описанию
...
- **Пересчитывать баланс** счета при обнаружении расхождений (автоматически или вручную)
...
|----------|------|----------|
| `BankAccount` | id, name, balance | Банковский счет. Баланс изменяется только через методы `deposit`/`withdraw` |
...
| Класс | Что проверяется |
|-------|----------------|
| `BankAccountServiceTest` | Создание сохраняет через репозиторий; получение существующего счета; получение несуществующего -> `EntityNotFoundException` |
...
| `AnalyticsServiceTest` | Пустой отчет (все по нулям); корректный подсчет доходов/расходов/разницы; группировка по категориям |
| `BalanceRecalculationServiceTest` | Нет расхождения -> 0; есть расхождение -> возвращает разницу; пересчет исправляет баланс |
...
| AnalyticsServiceImpl	| Аналитика и отчеты |
| BalanceRecalculationServiceImpl |	Пересчет баланса |
...
- Аналитика: новый вид отчета - новый метод в интерфейсе или новый сервис, существующий код не ломается
...
При удалении категории операции, ссылающиеся на нее по categoryId, остаются без родительского класса. Аналогично при удалении счета.
...
При импорте операция может ссылаться на несуществующий счет или категорию, если данные неполные или повреждены.
...
С абстракцией: сервисы зависят от BankAccountRepository. Создаем JpaBankAccountRepository - меняем один класс, все сервисы работают без изменений.
